...
leap:: Year -> Bool
leap y = (mod y 4 == 0) || (mod y 400 == 0) && not (mod y 100 == 0)
...
fstdays y = map (\x -> toEnum (mod x 7)::DayName) (filter (\x -> x < 360) (mtotals y))
 --buscar otra forma de omitir el ultimo elemento

fstday :: Month -> Year -> DayName
fstday m y = f (fstdays y) (fromEnum m::Int)
	where
		f (x:xs) 0 = x
		f (x:xs) n = f xs (n-1) 

day :: Day -> Month -> Year -> DayName
day d m y = toEnum(mod ((fromEnum (fstday m y)::Int) - 1  + d) 7)::DayName
